...
hooks 자체는 Python 스크립트라 AI를 호출하지 않습니다. 다만 경고 메시지가 AI 컨텍스트에 주입되면 그만큼 토큰을 씁니다.

| 기능 | hook 실행 | AI 컨텍스트 | 합계 |
|------|----------|------------|------|
| API 키 차단 | 0 | 차단 메시지 ~20 | **~20** |
| 실수 경고 | 0 | 경고 메시지 ~30 | **~30** |
| 패턴 매칭 경고 | 0 | 매칭 시 ~50, 없으면 0 | **0~50** |
| CLAUDE.md | - | 매 세션 1회 ~100 | **~100** |
| /sim | - | AI가 분석 ~500+ | **~500+** |

평소 사용: 매칭 없으면 CLAUDE.md 로딩 ~100만. 규칙 파일 수백 줄 읽는 시스템보다 적습니다.
